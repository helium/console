...
const FunctionButton = ({ id, name, type, selected, warn }) => (
  <React.Fragment>
...
export default ({ functions, shownFunctionId }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row' }}>
      {functions.map(f => (
        <FunctionButton warn={allowedFunctions && !allowedFunctions[f.format]} key={f.id} id={f.id} type={f.type} name={f.name} selected={f.id === shownFunctionId} />
      ))}
    </div>
  );
}
