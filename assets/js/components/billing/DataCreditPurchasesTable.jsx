...
class DataCreditPurchasesTable extends Component {
  ...
  componentDidMount() {
    ...
    this.channel.join();
    this.channel.on(
      `graphql:dc_purchases_table:${currentOrganizationId}:update_dc_table`,
      (message) => {
      ...
      },
      {
        ...
        render: (data, record) => {
          if (record.card_type == "transfer" && record.from_organization) {
            ...
            return (
              <Text>
                <CaretRightOutlined style={styles.redIcon} />
          ...
          if (record.card_type == "burn") {
            ...
            return (
              <PaymentCard
                key={record.id}
                id={record.id}
                card={{ brand: record.card_type, last4: record.last_4 }}
              />
            );
          }
        },
  ...
  render() {
    ...
    return (
      <Card title={title} bodyStyle={{ padding: 0, paddingTop: 1 }}>
        {this.renderTable()}
      </Card>
    );
  }
...
export default connect(
  ...
  withGql(DataCreditPurchasesTable, PAGINATED_DC_PURCHASES, (props) => ({
    fetchPolicy: "cache-first",
    variables: { page: 1, pageSize: 10 },
    name: "dcPurchasesQuery",
  }))
);
