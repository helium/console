    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text
            onClick={this.props.toggleNav}
            style={{ color: '#ffffff', fontSize: 16, position: 'relative', top: 1, cursor: 'pointer' }}
          >
            Menu
          </Text>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Dropdown overlay={menu(this.handleClick, currentOrganizationName)} trigger={['click']} onVisibleChange={visible => this.setState({ userMenuVisible: visible })}>
            <img draggable="false" src={this.state.userMenuVisible ? ProfileActive : ProfileInactive} style={{ height:30, marginLeft: 10, cursor: 'pointer' }}/>
          </Dropdown>
        </div>
