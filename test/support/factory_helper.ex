defmodule Console.FactoryHelper do
  ...
  def authenticate_user(%{conn: conn}) do
    {:ok, user, team, organization} = Console.Auth.create_user(params_for(:user, %{password: "password"}), %{name: "Test Team"}, %{name: "Test Organization"})
    token = Console.Auth.generate_session_token(user, organization)
    conn = conn
           |> put_req_header("accept", "application/json")
           |> put_req_header("authorization", "bearer: " <> token)
    {:ok, conn: conn, user: user, team: team, organization: organization}
  end

  def unauthenticated_user(%{conn: conn}) do
    {:ok, user, _, _} = Console.Auth.create_user(params_for(:user, %{password: "password"}), %{name: "Test Team"}, %{name: "Test Organization"})
